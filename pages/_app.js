...
export default function App({ Component, pageProps }) {
  ...
  useEffect(() => {
    // lifted to app to serve other pages
  ...
  function toggleFavorite(slug) {
    // ok this thing works in a way that

    const isAlreadyFavorite = favorites.includes(slug); // checks whether the artwork is included in the array of favs

    if (isAlreadyFavorite) {
      // if yes uses the filter thingy to create a new array without the fav artwork if its slug is the same as slug
